function Sout = learn_coefficients_hyperspectral(B, X, alpha, gamma, L, Sinit)
...
%         [Sout(:,i), fobj]= ls_featuresign_sub (B, S, X(:,i), BtB, BtX(:,i), L, i, alpha, gamma, sinit);
        [Sout(:,i), fobj]= ls_featuresign_sub (B, S, xi, BtB, BtX(:,i), L, i, alpha, gamma, sinit);
    else
%         [Sout(:,i), fobj]= ls_featuresign_sub (B, Sout, X(:,i), BtB, BtX(:,i), L, i, alpha, gamma);
        [Sout(:,i), fobj]= ls_featuresign_sub (B, Sout, xi, BtB, BtX(:,i), L, i, alpha, gamma);
    end
end
...
a = 0.5*sumTemp+0.5*alpha*L(idx,idx)*(s_new-s2)'*(s_new-s2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
f= 0.5*(sum_norm + alpha*L(i,i).*(s'*(s+2*P)));
f= f+ gamma*norm(s,1);
