    ...
    % Take a random permutation of the samples
        ...
        if t==1 || ~pars.reuse_coeff
            if strcmp(pars.sparsity_func, 'L1') || strcmp(pars.sparsity_func, 'LARS') || strcmp(pars.sparsity_func, 'FS')
                S= l1ls_featuresign(B, X, pars.beta/pars.sigma*pars.noise_var);
            else
                S= cgf_fitS_sc2(B, X, pars.sparsity_func, pars.noise_var, pars.beta, pars.epsilon, pars.sigma, pars.tol, false, false, false);
            end
            S(find(isnan(S)))=0;           
        else
            if strcmp(pars.sparsity_func, 'L1') || strcmp(pars.sparsity_func, 'LARS') || strcmp(pars.sparsity_func, 'FS')
                tic
                S= l1ls_featuresign(B, X, pars.beta/pars.sigma*pars.noise_var, S);
                FS_time = toc
            else
                S= cgf_fitS_sc2(B, X, pars.sparsity_func, pars.noise_var, pars.beta, pars.epsilon, pars.sigma, pars.tol, false, false, false, S);
            end
            S(find(isnan(S)))=0;
        end
        ...
        % get objective
        [fobj, fresidue, fsparsity] = getObjective2(B, S, X, pars.sparsity_func, pars.noise_var, pars.beta, pars.sigma, pars.epsilon);
        ...
        % update basis
        B = l2ls_learn_basis_dual(X, S, pars.VAR_basis);
...
%     end
    ...
    if mod(t,pars.save_every)==0 || t==pars.num_trials
        ...
        save(experiment.matfname, 't', 'pars', 'B', 'S','stat');
        fprintf('saved as %s\n', experiment.matfname);
    end
end
