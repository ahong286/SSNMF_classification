function Sout = learn_coefficients_cs(B, X, gamma, Sinit)
% The feature-sign search algorithm
% L1-regularized least squares problem solver
%
% This code solves the following problem:
% 
%   minimize_s 0.5*(sum(||xki-Bk*si||^2 + alpha*si'si + 2*alpha*si(sigma(Lijsj)) +
%   gamma*||si||_1 ,(k=1,...,K)
% X :          - data matrix, K-by-1 cell array containing K M-by-N data matrices.
% B :          - basis matrix, K-by-1 cell array containing K M-by-P basis matrices 
...
Sout= zeros(size(B{1},2), size(X{1},2));
% BtB = B'*B;
% BtX = B'*X;
BtB = zeros(size(B{1},2), size(B{1},2)); %sum(A'A)
BtX = zeros(size(B{1},2), size(X{1},2)); %sum(A'X)
K = length(B);
for k = 1:K
    BtX = BtX+B{k}'*X{k};
    BtB = BtB+B{k}'*B{k};
...
%     if mod(i, 100)==0, fprintf('.'); end %fprintf(1, 'l1ls_featuresign: %d/%d\r', i, size(X,2)); end
    ...
    if use_Sinit
        ...
        a = sum(sum(Sinit,2)==0);
        S = [Sout(:,1:i),Sinit(:,(i+1):size(X{1},2))];

%         [Sout(:,i), fobj]= ls_featuresign_sub (B, S, X(:,i), BtB, BtX(:,i), L, i, alpha, gamma, sinit);
        [Sout(:,i), fobj]= ls_featuresign_sub (B, S, xi, BtB, BtX(:,i),i, gamma, sinit);
    else
%         [Sout(:,i), fobj]= ls_featuresign_sub (B, Sout, X(:,i), BtB, BtX(:,i), L, i, alpha, gamma);
        [Sout(:,i), fobj]= ls_featuresign_sub (B, Sout, xi, BtB, BtX(:,i),i, gamma);
    end
end
...
function [s, fobj] = ls_featuresign_sub (B, S, x, BtB, Btx, i, gamma, sinit)
...
fobj = fobj_featuresign(s, B, x, BtB, Btx, gamma);
...
for iter=1:ITERMAX
    ...
    while 1
        ...
        [s, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (s, B, x, BtB, Btx, i, theta, act, act_indx1, gamma);
...
function [s, theta, act, act_indx1, optimality1, lsearch, fobj] = compute_FS_step (s, B, x, BtB, Btx, idx, theta, act, act_indx1, gamma)
...
s_new = (BtB2)\ ( Btx(act_indx1) - gamma.*theta2); % RR
...
K = length(B);
sumTemp = sum((B{1}(:,act_indx1)*(s_new-s2)).^2);
for k = 2:K
    sumTemp = sumTemp + sum((B{k}(:,act_indx1)*(s_new-s2)).^2);
end
a = 0.5*sumTemp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
b= (s2'*BtB2*(s_new-s2) - (s_new-s2)'*Btx(act_indx1));
...
%fobj_new = 0; 
fobj_new = fobj_featuresign(s, B, x, BtB, Btx, gamma);
...
function [f, g] = fobj_featuresign(s, B, x, BtB, Btx, gamma)

% f= 0.5*(norm(x-B*s)^2+alpha*L(i,i).*(s'*(s+2*P)));
K = length(x);
sum_norm = norm(x{1}-B{1}*s)^2;
for k = 2:K
    sum_norm = sum_norm + norm(x{k}-B{k}*s)^2;
end

f= 0.5*sum_norm + gamma*norm(s,1);
