...
% vgt_train = reshape(gt_train, [numel(gt_train),1]);
% vgt_test = reshape(gt_test, [numel(gt_test),1]);
% vgt_train = double(vgt_train);
% vgt_test = double(vgt_test);
...
if(isTrainDictionary==1)
    ...
    savefile = sprintf('%s_%s_patches%d_nBasis%d_beta%0.2f_KNN%d_whole.mat',datasetname,method, patch_size(1),nBasis,beta, options.k);
