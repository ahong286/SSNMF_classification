function W = constructW_HSI(fea,options)
%	Usage:
        ...
        case {lower('Binary')}
            if options.k > 0
                ...
                for i=1:nLabel
                    ...
                    idx_num = options.k+1;
                    if(idx_num > length(classIdx))
                        idx_num = length(classIdx);
                    end
                    idx = idx(:,1:idx_num);
                    
                    nSmpClass = length(classIdx)*(idx_num);
                    G(idNow+1:nSmpClass+idNow,1) = repmat(classIdx,[idx_num,1]);
