function [B S stat] = GraphSC_hyperspectral(X, W, num_bases, alpha, beta, num_iters, Binit, pars)
...
while t < pars.num_iters
    ...
    if t ==1
        S= learn_coefficients_hyperspectral(B, X, alpha, pars.beta/pars.sigma*pars.noise_var,L);
    else
        S= learn_coefficients_hyperspectral(B, X, alpha, pars.beta/pars.sigma*pars.noise_var, L, S);
    end
    S(isnan(S))=0;
